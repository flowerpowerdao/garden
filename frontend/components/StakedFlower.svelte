...
<FlowerPreview {collection} {tokenIndex}>
  {#if 'DissolveDelay' in neuron.dissolveState}
    <Button on:click={toggleModal}>Dissolve</Button>
  {:else if 'DissolveTimestamp' in neuron.dissolveState}
    <Button on:click={toggleModal}>Restake</Button>
  {/if}
</FlowerPreview>
